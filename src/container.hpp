...
struct ShardPayload {
    std::vector<uint8_t> cbuf;   // zstd-compressed bytes
    uint32_t raw_sz;
    uint32_t n_records;
    uint32_t min_sstart;
    uint32_t max_send;
...
// Serialize local contig dict + VarNT block and compress.
// Modifies recs (remaps contig_idx to local space). Returns compressed payload.
inline ShardPayload compress_shard_payload(
    ...
    ShardPayload sp;
    sp.n_records = uint32_t(recs.size());
    sp.min_sstart = UINT32_MAX; sp.max_send = 0;
    for (const auto& r : recs) {
        sp.min_sstart = std::min(sp.min_sstart, r.sstart);
        sp.max_send   = std::max(sp.max_send,   r.send);
    }
...
inline void write_shard_block(int fd,
    ...
    auto sp = compress_shard_payload(global_contigs, recs, zstd_level);
    const auto& cbuf = sp.cbuf;
    size_t csz = cbuf.size();
    ...
    hdr_bytes.push_back(0); hdr_bytes.push_back(0); hdr_bytes.push_back(0);
    write_u32(hdr_bytes, uint32_t(csz));
    write_u32(hdr_bytes, sp.raw_sz);
    write_u32(hdr_bytes, sp.n_records);
    write_u32(hdr_bytes, sp.min_sstart);
    write_u32(hdr_bytes, sp.max_send);
